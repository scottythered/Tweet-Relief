...
def main():
    ...
    creds_parser = parser.add_argument_group('First, enter your Twitter API credentials:',
    ...
    date_parser.add_argument('stop_date', metavar='Tweets published before this date will be deleted:',
                             widget='DateChooser')

    keepers = parser.add_argument_group('Keeping any tweets?')
    keepers.add_argument('keepers', default='None', metavar='Enter tweet IDs you want to keep, separated by commas. '
...
def destroyer(tweet_objects, api_object, arg_2):
    ...
    cutoff_date = datetime.strptime(arg_2, '%Y-%m-%d')
    destroyed = []
